...
/**
 * Account Executive New Starts View
 *
 * Personal new starts tracking for sales reps
 * - View their own new starts entries
 * - Log new start entries (link to /ae/new-starts/new)
 * - Track handoff status to operations
 */
export default function AENewStartsView() {
  ...
  // Fetch new starts from BigQuery
  // Uses ae-new-start-entries which filters by salesPerson automatically
  const {
    data: entries,
    ...
    filters: { daysBack, limit: 500 },
    defaultData: EMPTY_ENTRIES,
    transformBigQueryData: (data) => data,
    includeRoleFilters: true, // Auto-inject salesPerson filter
  })
  ...
  return (
    <TooltipProvider delayDuration={0}>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">My New Starts</h1>
          <p className="text-gray-500 dark:text-gray-400">Track your new customer installations (PestPac + Salesforce)</p>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap gap-6 text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-red-700 dark:text-red-400">Your Sales Data (PestPac/Salesforce)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-4 h-4 bg-yellow-100 dark:bg-yellow-950 border-2 border-yellow-400 dark:border-yellow-700 rounded" />
                <span className="font-medium text-yellow-700 dark:text-yellow-400">Operations Status (Updated by Ops Manager)</span>
              </div>
            </div>
            <p className="text-xs text-muted-foreground">
              Data from BigQuery • Click &quot;Log New Start&quot; to add entries
            </p>
          </div>
        </CardContent>
      </Card>
